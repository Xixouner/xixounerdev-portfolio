...
export const metadata: Metadata = {
  ...
  description:
    "Découvrez mes projets : sites vitrine, applications web, migrations DevOps. Des vraies réalisations avec des vrais chiffres.",
  openGraph: {
    type: "website", locale: "fr_FR", url: "https://xixouner.com/realisations",
    siteName: "XixounerDev", title: "Réalisations — Portfolio Développeur Web | XixounerDev",
    description: "Sites vitrine, apps web, migrations DevOps : mes projets avec chiffres et résultats.",
  },
      ...
      <div className="mx-auto max-w-6xl px-6">
        <Reveal variant="fadeUp">
          ...
          <p className="mt-3 max-w-2xl text-text-light">
            Des projets concrets, des vrais chiffres, zéro bullshit. Voici ce que je fais pour mes clients.
          </p>
        </Reveal>
              ...
              <article className="grid gap-8 lg:grid-cols-5">
                <div className="lg:col-span-2">
                  <div className="overflow-hidden rounded-2xl border border-border">
                    <img src={project.image} alt={project.title} className="w-full object-cover" loading="lazy" width={600} height={400} />
                  </div>
                </div>
                <div className="lg:col-span-3">
                  <span className={`inline-block rounded-full px-3 py-1 text-xs font-medium ${getCatColor(project.category)}`}>
                    {project.category}
                  </span>
                  ...
                  <ul className="mt-1 space-y-1">
                    {project.solution.map((s) => (
                      <li key={s} className="flex items-start gap-2 text-sm text-text-light">
                  ...
                  <div className="mt-2 grid grid-cols-2 gap-2 sm:grid-cols-4">
                    {project.results.map((r) => (
                      <div key={r.label} className="rounded-lg bg-accent/5 px-3 py-2 text-center">
                  ...
                  <div className="mt-3 flex flex-wrap gap-1.5">
                    {project.techs.map((t) => (
                      <span key={t} className="rounded-md bg-border/50 px-2 py-0.5 text-xs text-text-light">{t}</span>
                    ))}
                  </div>
                  {project.testimonial && (
                    <blockquote className="mt-4 border-l-4 border-accent pl-4 italic text-text-light">
                      « {project.testimonial.text} »
                      <footer className="mt-1 text-xs font-medium not-italic text-text-muted">
        ...
        <Reveal variant="fadeUp" delay={0.4}>
          <div className="mt-20 rounded-2xl border border-accent/30 bg-gradient-to-br from-accent/5 to-accent/10 p-10 text-center">
            ...
            <p className="mt-2 text-text-light">
              Le prochain projet que je mets en avant, c&apos;est peut-être le vôtre. Parlons-en.
            </p>
